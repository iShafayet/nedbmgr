...
(->
  ...
  dateFormat = do ->
    ...
    (date, mask, utc) ->
      ...
      if arguments.length == 1 and Object::toString.call(date) == '[object String]' and !/\d/.test(date)
        mask = date
        date = undefined
      ...
      flags =
        ...
        S: [
          ...
          'nd'
          'rd'
        ][if d % 10 > 3 then 0 else (d % 100 - (d % 10) != 10) * d % 10]
  ...
  # @templates = dateFormat.masks
  # @templates['date'] = 'yyyy-mm-dd'
  # @templates['time'] = 'HH:MM:ss'
  # @templates['datetime'] = 'yyyy-mm-dd HH:MM:ss'

).call window['date-format-library']
